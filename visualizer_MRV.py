...
class SudokuSolver: #implements the logic for solving the puzzle using backtracking
    ...
    def find_most_constrained_empty(self):
        """Find the most constrained empty cell (fewest valid numbers)."""
        min_options = self.size + 1  # More than the maximum possible options
        best_cell = None
        for row in range(self.size):
            for col in range(self.size):
                if self.board[row, col] == 0:  # Only consider empty cells
                    valid_count = sum(1 for num in range(1, self.size + 1) if self.is_valid(row, col, num))
    ...
    def solve(self):
        ...
        empty = self.find_most_constrained_empty()
        if not empty:
